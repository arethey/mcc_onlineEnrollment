 <form action="" method="GET">
     ...
     <div class="row invoice-info">
        ...
        foreach ($cur as $result) {
          ...
          echo '<option '. (isset($_GET['Course']) && $_GET['Course']==$course_val? 'selected="selected"' : '') .' value="'.$course_val.'" >'.$result->COURSE_NAME.'-'.$result->COURSE_LEVEL.' </option>';
      ...
      ?>
         ...
         <div class="col-sm-2 invoice-col">
             ...
             <address>
                 <select name="Semester" class="form-control">
                     <option
                         <?php if(isset($_GET['Semester']) && $_GET['Semester']=='First') { echo 'selected="selected"'; } ?>
                         value="First">
                         First</option>
                     <option
                         <?php if(isset($_GET['Semester']) && $_GET['Semester']=='Second    ') { echo 'selected="selected"'; } ?>
                         value="Second">
                         Second</option>
                 </select>
             </address>
        ...
        foreach ($cur as $result) {
          echo '<option '.(isset($_GET['SY']) && $_GET['SY']==$result->AY ? 'selected="selected"' : '').'>'.$result->AY.'</option>';
         ...
         <div class="col-xs-12">
             <h2 class="page-header">
                 ...
                 <small class="pull-right">
                     <?php echo (isset($_GET['Course'])) ? 'Course/Year :' .$_GET['Course'] .' ||': ''; ?>
                     <?php echo (isset($_GET['Semester'])) ? ' Semester :' .$_GET['Semester'] .' ||': ''; ?>
                     <?php echo (isset($_GET['SY'])) ? ' SY :' .$_GET['SY'] : ''; ?>
                 </small>
             </h2>
         </div>
     </div>
         ...
         <div class="col-xs-12 col-md-12 table-responsive">
             <table class="table table-bordered table-striped" style="font-size:11px" cellspacing="0">
                 ...
                 <tbody>
                     <?php
                $tot = 0;
               if(isset($_GET['Course']) && isset($_GET['Semester']) && isset($_GET['SY'])){ 
            ...
            if ($_GET['Course']=='All') {
              # code...
                  ...
                  $sql ="SELECT * FROM schoolyr sy, `tblstudent`  s ,`course` c 
                        ...
                        AND s.SEMESTER  LIKE '%" . $_GET['Semester'] ."%'  AND sy.AY = '" .$_GET['SY']. "'";
              ...
              ?>
                 ...
                 $sql ="SELECT * FROM `schoolyr` sy, `tblstudent`  s ,`course` c 
                        WHERE sy.`IDNO`=s.`IDNO` AND s.`COURSE_ID`=c.`COURSE_ID` AND CONCAT(COURSE_NAME,'-',COURSE_LEVEL) LIKE '%" . $_GET['Course'] ."%' 
                        AND sy.SEMESTER  LIKE '%" . $_GET['Semester'] ."%'  AND sy.AY = '" .$_GET['SY']. "'";
